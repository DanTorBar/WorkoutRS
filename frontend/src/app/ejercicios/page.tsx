...
export default function EjerciciosPage() {
  ...
  const [equipmentOptions, setEquipmentOptions] = useState([
    { value: "", label: "Todos los equipamientos" },
  ]);

  // Obtener todos los valores únicos de equipment de los ejercicios cargados
  const equipmentOptionsAll = Array.from(
    new Set(
      ejercicios.map((e: any) => e.equipment).filter((eq) => eq && eq !== "N/A")
    )
  );

  const fetchMoreEjercicios = async () => {
    try {
      ...
      const newEjerciciosData = await res.json();
      if (newEjerciciosData.results.length < PAGE_SIZE) setHasMore(false);
      setEjercicios((prev: any[]) => [...prev, ...newEjerciciosData.results]);
      setPage((prev) => prev + 1);
    } catch {}
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!ejRes.ok) throw new Error("Error al cargar ejercicios");
        const ejerciciosData = await ejRes.json();
        let favsData: any[] = [];
        if (token) {
        ...
        setEjercicios(ejerciciosData.results);
        setFavs(favsData);
      } catch (e: any) {
        setError(e.message);
      } finally {
    ...
    fetchData();
  }, [search, order, category, muscle, equipment]);
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}exercises/options/`
        );
        ...
        setMuscleOptions([
          { value: "", label: "Todos los músculos" },
          ...data.muscles
            .filter((m: string) => m !== "N/A")
            .map((m: string) => ({ value: m, label: m.replace(/,/g, ", ") })),
        ]);
        setEquipmentOptions([
          { value: "", label: "Todos los equipamientos" },
          ...data.equipment.map((eq: string) => ({ value: eq, label: eq })),
        ]);
      } catch (e) {
  ...
  return (
    <main className="min-h-screen bg-bg text-text p-8">
      <h1 className="text-3xl font-bold mb-6">Listado de Ejercicios</h1>
      <form
      ...
      >
        ...
        <div className="relative">
          ...
          <select
            value={equipment}
            onChange={(e) => setEquipment(e.target.value.toLowerCase())}
            className="pl-10 pr-4 py-2 rounded-lg border border-border bg-surface text-text shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition-all duration-200 cursor-pointer hover:border-primary"
          >
      ...
      <InfiniteScroll
        ...
        next={fetchMoreEjercicios}
        hasMore={hasMore}
        loader={
          hasMore && !loading && ejercicios.length > 0 ? (
            <div className="text-center py-4">Cargando más ejercicios...</div>
          ) : null
        }
        endMessage={
            ...
            : ejercicios.length > 0 &&
              ejercicios.map((ejercicio: any) => {
                const liked = favs.some(
                  (fav: any) => fav.exercise === ejercicio.id
                );
                const favCount = ejercicio.likes_count || 0;
                return (
                  <li
                    key={ejercicio.id}
                  ...
                  >
                    <div className="flex-1">
                      ...
                      <div className="mt-4">
                        {(ejercicio.priMuscles || ejercicio.secMuscles) &&
                          (ejercicio.priMuscles[0].name !== "N/A" ||
                            ejercicio.secMuscles[0].name !== "N/A") && (
                            <div className="flex flex-wrap gap-2 mt-2 mb-4">
                              {/* Músculos principales */}
                              {ejercicio.priMuscles &&
                                ejercicio.priMuscles.length > 0 &&
                                ejercicio.priMuscles[0].name !== "N/A" && (
                              ...
                              {ejercicio.secMuscles &&
                                ejercicio.secMuscles.length > 0 &&
                                ejercicio.secMuscles[0].name !== "N/A" && (
                        ...
                        <div className="flex flex-wrap gap-2 mt-1">
                          ...
                          <span className="inline-flex items-center bg-primary/10 text-primary px-2 py-1 rounded-full text-xs font-medium">
                            ...
                            {ejercicio.exerciseCategory &&
                              ejercicio.exerciseCategory.replace(/,/g, ", ")}
                          </span>
                          {/* Ambiente (Casa, Gimnasio, Aire Libre) */}
                          {(() => {
                            if (!ejercicio.equipment) return null;
                            const ambientes = [
                              "Casa",
                              "Gimnasio",
                              "Aire Libre",
                            ...
                            const eqArr = ejercicio.equipment
                              .split(",")
                              .map((e: string) =>
                                e.trim().replace(/AireLibre/, "Aire Libre")
                              );
                            return ["Casa", "Gimnasio", "Aire Libre"]
                              .filter((amb) => eqArr.includes(amb))
                              .map((amb) => (
                                <span
                                  key={amb}
                                  className="inline-flex items-center bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full text-xs font-medium"
                                >
                                  <svg
                                    viewBox="0 0 24 24"
                                    fill="none"
                          ...
                          {/* Equipamiento (sin ambientes, sin 'Ninguno') */}
                          {(() => {
                            if (!ejercicio.equipment) return null;
                            const ambientes = [
                              "Casa",
                            ...
                            const eqFiltered = eqArr.filter(
                              (e: string) =>
                                !["Casa", "Gimnasio", "Aire Libre"].includes(
                                  e
                                ) && e !== "Ninguno"
                            );
                            return eqFiltered.length > 0
                              ? eqFiltered.map((eq: string) => (
                                  <span
                                    key={eq}
